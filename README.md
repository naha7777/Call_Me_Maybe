...
- Logit Masking: I retrieve the model's probabilities (logits) for all tokens and set the probability of invalid tokens to −∞, or provide a list of valid tokens, automatically excluding all others
...
- Strict Typing: I use mypy to ensure type-safe communication between my code and the model SDK.
- Dynamic Argument Extraction: I implemented specific states (param, string, ...) to handle the extraction of varied data types (floats, strings) from the prompts.
...
- Speed: Processing time is negligible for standard test sets
...
- SDK Integration: I resolved module resolution issues with Mypy by correctly configuring PYTHONPATH and adding missing type stubs.
...
- [UV Package Manager Guide](https://www.datacamp.com/fr/tutorial/python-uv?dc_referrer=https%3A%2F%2Fwww.google.com%2F)
...
- explanations about LLM, models, tokens
- makefile debugging
